...
function init(){
  particles = Array.from({length: Math.max(60, Math.floor(w*h/25000))}, ()=> ({
    x: Math.random()*w,
    y: Math.random()*h,
...
function step(){
  ...
  g.addColorStop(0,'#0b0c10'); g.addColorStop(1,'#0d1117');
  ...
  for (let i=0;i<particles.length;i++){
    ...
    ctx.beginPath(); ctx.arc(p.x,p.y,p.r,0,Math.PI*2); ctx.fillStyle = 'rgba(125,211,252,0.8)'; ctx.fill();
    for (let j=i+1;j<particles.length;j++){
      const q = particles[j]; const dx=p.x-q.x, dy=p.y-q.y, d=dx*dx+dy*dy;
      if (d<120*120){ ctx.strokeStyle = 'rgba(125,211,252,0.08)'; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(p.x,p.y); ctx.lineTo(q.x,q.y); ctx.stroke(); }
    }
  }
